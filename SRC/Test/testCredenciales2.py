...
class TCCredenciales(unittest.TestCase):
    ...
    def test_credenciales_vista(self):
        ...
        self.page_credenciales.click_detalle_volver()
        time.sleep(3)
        self.driver.find_element(MobileBy.ANDROID_UIAUTOMATOR,
                                 "new UiScrollable(new UiSelector().scrollable(true)).setAsHorizontalList()" +
                                 ".scrollIntoView(new UiSelector().text(\"ABITANTE, VANINA PAOLA\"))")

        self.page_credenciales.click_vista_credencial()
        self.assertEqual(self.page_credenciales.return_du_detalle_credencial_2(), "23326494")
